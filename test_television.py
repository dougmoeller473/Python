...
class Test:
    ...
    def test_power(self):
        self.tv1.power() #Powering on
        assert self.tv1.__str__() == 'Power = True, Channel = 0, Volume = 0'

        self.tv1.power() #Powering off
        assert self.tv1.__str__() == 'Power = False, Channel = 0, Volume = 0'


    def test_mute(self):
        self.tv1.power()  #Powering on(T, 0, 0)
        self.tv1.volume_up() #Volume up(T, 0, 1)
        self.tv1.mute() #Volume muted(T, 0, 0)
        assert self.tv1.__str__() == 'Power = True, Channel = 0, Volume = 0'
        self.tv1.mute() #Volumbe unmuted(T, 0, 1)
        assert self.tv1.__str__() == 'Power = True, Channel = 0, Volume = 1'
        self.tv1.power() #Powering off(F, 0, 1)
        self.tv1.mute() #Volume muted(F, 0, 1)
